...
# Name: Ronith Ranjan and Derrick Liang and Alex Downey    Date: 12/2/2018
...
class CustomPlayer():
    ...
    def move(self, game, legal_moves, time_left):
        ...
        best_move, best_queen, utility = self.minimax(game, time_left, alpha, beta, depth=self.search_depth) # TODO ADD ALPHA BETA
    ...
    def minimax(self, game, time_left, alpha, beta, depth=float("inf"), maximizing_player=True):
        # TODO: finish this function!
        best_val = 0
        best_move = None
        best_queen = None
        ...
        if maximizing_player:
            ...
            for m in moves:
                ...
                value = self.minimax(tmp, time_left, alpha, beta, depth-1, maximizing_player = False)
                if value[2] > best_val:
                    best_val = value[2]
                    best_move = m
                if best_val >= beta:  # this means that our potential value will never be selected by the parent, since parent is a min
            ...
            for m in moves:
                tmp = game.forecast_move(m, game.get_active_players_queen())
                value = self.minimax(tmp, time_left, alpha, beta, depth-1, maximizing_player = True)
                if value[2] < best_val:
                    best_val = value[2]
                    best_move = m

                if best_val <= alpha:  # we need to do at least as well as the best so far. Otherwise, parent won't pick us
                    return best_move, 1, best_val
                beta = min(beta, best_val)  # continually shrink the opponents benefit'''
                if beta <= alpha:
                    break
