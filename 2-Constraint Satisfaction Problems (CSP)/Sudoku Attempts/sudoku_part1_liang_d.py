...
def recursive_backtrack(assignment):
    if is_complete(assignment):
        return assignment
    var = select_minimum_remaining_values_var(assignment)  # TODO you can swap this for any of the three selection methods
...
def tie_break_MRV(tied_variables, assignment):
    constraints = {
        var: sum([1 if is_unassigned(adj, assignment) else 0 for adj in adjacencies[var]]) for var in
        tied_variables
    }
