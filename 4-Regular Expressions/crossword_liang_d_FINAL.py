...
def setup_board():  # TODO not sure what the inputs are
    ...
    line = sys.argv
    line = ['crossword_Liang_D.py', '5x5', '0', 'twentyk.txt']
    line[1] = input("Dimensions: ")
    line[2] = input("Number of blocked squares: ")
    while input("Initial word (y/n)? ") in ['y', 'Y', 'yes']:
        line.append(input("Initial placement: "))
    ...
    return copy, fixed_words, input_file
...
def setup_structures(xw, fixed_words, input_file):
    ...
    words = {}
    current_domains = {
        (pos, orientation): [] for pos in range(len(xw)) if xw[pos] != BLOCKCHAR for orientation in ['H', 'V']
    }
    transposed = transpose(xw)
    revert_width(xw)
    ...
    for i in range(2):
        ...
        for match in matches:
            ...
            for j in range(length):
                current_domains[(pos + j * increment, orientation)] = curr_domain
                words[(pos + j * increment, orientation)] = [length, pos, word]
    ...
    for element in words:
        pos, orient = element
        update_letter_domains(xw, pos, orient, words, letter_domains, current_domains)
    ...
    return words, letter_domains, current_domains
...
def place_letter(xw, pos, char, words, letter_domains, current_domains, visited):
    ...
    for i in range(2):
        o = orientations[i]
        increment = increments[i]
        length, index, current = words[(pos, o)]
        for j in range(length):  # Need to update all letters within the word
            p = index + j*increment
            if (p, o) in words:  # We can't update it if it's been deleted already
                words[(p, o)][2] = replace(current, (pos - index)//increment, char)

    # Do a quick check: if we place a letter and it ends up that the final word is not within the current_domain of the
    # Other directions word, then return None. There is not way to check for this due to the nature of how letters were
    # Added

    if pos in letter_domains:
    ...
    for i in range(2):
        ...
        p = pos
        if (length, (pos - index) // increment, char) not in word_domains:  # Weird case happens, j is NEVER 2nd letter of 5 letter word, so break
            return None, 0, 0, 0, 0
        elif len(current_domains[(p, o)]) == 0:  # if it is clean, need to give it something to start with
            current_domains[(p, o)] = set(
                word_domains[(length, (pos - index) // increment, char)])  # use the RELATIVE position
        else:  # intersect it with the other
            other = set(word_domains[(length, (pos - index) // increment, char)])
            current_domains[(p, o)] = [x for x in current_domains[(p, o)] if x in other]
            #print("UPDATED", (p, o), current_domains[(pos, o)])

        for j in range(length):  # We need to update each positions current_domain. After the first, we know the rest!
            p = index + j*increment  # Go through from the start of the word; update each to match newest word domain
            if (p, o) in current_domains:  # Some positions may be deleted, so we need to be careful
                current_domains[(p, o)] = current_domains[(pos, o)]  # All positions within the word are the same
                #print("UPDATED", (p, o), current_domains[(p, o)])

        # Also check if the word is actually finished
        if OPENCHAR not in current:
            if current not in current_domains[(p, o)]:
                return None, 0, 0, 0, 0  # This means we placed it and it was valid in one direction, but not the other
            elif current not in visited:  # The word we have reached is something NEW
                completed_word = (True, current, o)  # from the words list, otherwise we'd be pruning for no reason
                ...
                for j in range(length):  # Delete these parts so that we don't have to keep updating them
                    p = index + j * increment
                    to_delete.append((p, o))
            elif current in visited:
                return None, 0, 0, 0, 0  # We accidentally visited this! BACKTRACK! TODO how would we improve this
    ...
    if completed_word[0]:  # Now we need to update the domains of all other words
        orientation = completed_word[2]
        length, index, current = words[(pos, o)]
        increment = 1 if orientation == 'H' else true_width
        deleted_positions = set(range(index, index+length*increment, increment))
        for key, value in words.items():  # now smaller by length!
            p, o = key
            if not (p in deleted_positions and o == orientation):
                current_domain_pos = current_domains[(p, o)]
                if current in current_domain_pos:
                    current_domain_pos.remove(current)
                    if len(current_domain_pos) == 0:
                        return None, 0, 0, 0, 0  # this means we precluded a location!!! Backtrack time!
    ...
    # Update letter_domains
    for o in orientations:
        update_letter_domains(xw, pos, o, words, letter_domains, current_domains)

    # Now, we can delete things
    for p, o in to_delete:
        del words[(p, o)]
        del current_domains[(p, o)]
    #for word in words.items():
     #   print(word)
    return xw, words, letter_domains, current_domains, visited  # updated version
...
def update_letter_domains(xw, pos, orient, words, letter_domains, current_domains):
    global width
    true_width = width + 2
    increment = 1 if orient == 'H' else true_width
    length, index, current = words[(pos, orient)]
    current_domain = current_domains[(pos, orient)]
    for j in range(length):
...
def copy_words(words):
    ...
    for key, value in words.items():
        length, index, current = value
        dict[key] = [length, index, current]
    return dict


def copy_domains(domains):
    return {
        key: set(list(value)[:]) for key, value in domains.items()
    }


def solve_puzzle(xw, words, letter_domains, current_domains, visited):
    global width, word_domains
    #  Failed case
    if xw == None:
        return None
    #  Finished board
        ...
        for letter in letters:
            tup = place_letter(xw, pos, letter, copy_words(words), copy_domains(letter_domains), copy_domains(current_domains), set(list(visited)[:]))
            # tup = xw, words, letter_domains, current_domains, visited
            if tup[0] != None:
                result = solve_puzzle(tup[0], tup[1], tup[2], tup[3], tup[4])
                if result != None:
                    return result
    return None
...
def main():
    global word_domains
    xw, fixed_words, input_file = setup_board()
    words, letter_domains, current_domains = setup_structures(xw, fixed_words, input_file)
    ...
    cur_time = time.time()
    xw = solve_puzzle(xw, words, letter_domains, current_domains, visited)
    display_clean(xw, False)
    next_time = time.time()
